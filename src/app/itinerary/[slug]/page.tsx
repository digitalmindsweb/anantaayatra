...
import { getItineraryBySlug, getItineraries, mapItineraryToContent } from "@/lib/itineraries";
import { MapPin, Clock, ArrowLeft, ArrowRight, Route } from "lucide-react";
import Link from "next/link";
import { redirect } from "next/navigation";

export default async function ItineraryPage({ params }: any) {
  const { slug } = await params;

  // 🔹 Get current itinerary
  const data = await getItineraryBySlug(slug);
  if (!data) redirect("/itinerary");

  const itinerary = mapItineraryToContent(data);

  // 🔹 Get all itineraries (for prev/next)
  const allRaw = await getItineraries();
  const all = allRaw.map(mapItineraryToContent);

  const currentIndex = all.findIndex((p) => p.slug === slug);

  const prevItinerary = currentIndex < all.length - 1 ? all[currentIndex + 1] : null;
  const nextItinerary = currentIndex > 0 ? all[currentIndex - 1] : null;
            ...
            {"dayWisePlan" in itinerary && (
              <div className="mb-12">
                ...
                <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 dark:before:via-slate-700 before:to-transparent">
                  ...
                  {itinerary.dayWisePlan.map((dayPlan: any, index: number) => (
                    <div
                      key={index}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-stretch gap-6 mb-12">
              ...
              {prevItinerary ? (
                <Link href={`/itinerary/${prevItinerary.slug}`} className="group flex-1 flex flex-col w-full text-left bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/30 dark:hover:bg-slate-800 p-6 rounded-2xl transition-colors border border-transparent dark:border-slate-700">
                  ...
                  <span className="font-serif font-bold text-lg text-slate-900 dark:text-white line-clamp-2">
                    {prevItinerary.title}
                  </span>
                </Link>
              ) : (
